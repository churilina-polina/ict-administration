...
Задание звучит так: взять простой HTTP-сервис и наблюдать за всякими штуками — метрики, логи, трейсы и алерты. Мы примерно понимали, что такое сервис, но что такое «наблюдаемость» и зачем к одному приложению цеплять семь сторонних программ — сначала было не понятно. Разобрались по ходу, и вот к чему пришли.
...
Всё это мы подняли локально через Docker Compose. Одна команда — работает 8 контейнеров: наш сервис и 7 инструментов вокруг него.
...
### Из чего собран стек
Нам важно прописать в отчете и теорию, чтобы мы сами поняли, что происходит и для чего это все нужно:)
...
Весь сервис — один файл `service/app.py`. Взяли Python и FastAPI, потому что мы только питон нормально знаем и много готовых примеров под OpenTelemetry)))
...
- `http_request_duration_seconds` — гистограмма, раскладывает запросы по промежуткам времени. Из неё считается **Duration** и перцентили.

Обновляет обе метрики один кусок кода — middleware `observe`, который оборачивает **каждый** запрос: засекает время в начале, после ответа увеличивает счётчик, кладёт длительность в гистограмму и пишет строку лога.
...
`trace_id` берётся из текущего спана(один шаг трейса) OpenTelemetry. Именно по нему потом можно взять строчку из логов и открыть ровно этот же запрос в Jaeger. Логи пишутся в два места: в консоль контейнера и в файл `/logs/app.log` — из файла их забирает Promtail.
...
Нас порадовало, что Grafana умеет настраивать себя из файлов при старте — не нужно кликать мышкой)
Папка `grafana/provisioning/`:
...
Тут был неочевидный момент: авто-инструментирование даёт только один спан на весь запрос. Всё, что интереснее — вложенные шаги и пометку ошибки — надо расставлять в коде своими ручками(клодиком):
...
Из вариантов доставки (webhook / Slack / email) взяли **webhook** — не нужен ни аккаунт в Slack, ни почтовый сервер. Уведомления уходят на контейнер `webhook`, который просто печатает всё присланное в свой лог. Это удобненько для проверки. `send_resolved: true` — слать уведомление ещё и когда алерт погас.
...
Взяли именно RED, потому что это минимальный набор, который работает для любого сервиса типа «запрос — ответ», и не зависит от того, что этот сервис делает. Три числа отвечают на три разных вопросика:
...
Ещё один плюс RED: эти же три метрики придётся настраивать в каждой следующей лабе курса, так что разобраться в них сейчас было бы супер...
...
Сервис «тормозит»: медленная база, внешний API не отвечает, не хватает процессора. Порог 1.5 секунды — отклик уже ощущается как медленный. Взяли p95, чтобы заметить проблему на «хвосте» раньше, чем она затронет большинство запросов. `warning`, потому что сервис ещё отвечает, просто плохо.
...
Еще пользовались AI-ассистентом(`AGENTS.md` / `CLAUDE.md`) — для генерации каркаса конфигов и кода сервиса и для пошагового разбора очень незнакомых компонентов:)
